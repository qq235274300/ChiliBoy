...
void URuntimeMeshBooleanTool::Shutdown(EToolShutdownType ShutdownType)
{
	...
	if (ShutdownType == EToolShutdownType::Accept)
	{
		...
		UInteractiveMeshSceneObject* SO = UInteractiveMeshSceneSubsystem::Get()->FindSceneObjectByActor(KeepActor);
		UInteractiveMeshSceneSubsystem::Get()->SetSelected(SO, true, false);
		UInteractiveMeshSceneSubsystem::Get()->DeleteSelectedSceneObjects(KeepActor);
