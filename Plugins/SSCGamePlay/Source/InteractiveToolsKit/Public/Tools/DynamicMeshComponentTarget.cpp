...
#include "SubSystem/InteractiveToolsSubsystem.h"  // to emit change transaction
...
void FSimpleDynamicMeshComponentTarget::CommitMesh(const FCommitter& ModifyFunc)
{
	...
	UInteractiveToolsSubsystem::Get()->GetTransactionAPI()->AppendChange(MeshComponent, MoveTemp(ReplaceChange),
